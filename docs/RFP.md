...
| 기능 ID | 구분 | 기능 설명 |
|---------|------|-----------|
| BLE-01 | 연결 | 앱 실행 시 자동 스캔 → 최근 페어링 디바이스 우선 연결 |
| BLE-02 | 연결 | 백그라운드 재연결 지원 |
| BLE-03 | 연결 | 연결 상태 UI 표시 (연결됨/해제됨/연결중) |
| BLE-04 | 수신 | +1 카운트 이벤트 실시간 수신 및 DB 업데이트 |
| BLE-05 | 수신 | BATTERY(level%) 정보 수신 및 UI 갱신 |
| BLE-06 | 수신 | RESET 이벤트 수신 (앱 일일 누적은 유지, 로그만 기록) |
| BLE-07 | 오프라인 | 연결 끊어져도 로컬 DB 유지, 재연결 시 반지의 내부 카운트가 있다면 증분만 합산 |
